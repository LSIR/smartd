...
public class ControllerServlet extends HttpServlet {
   ...
   public static final int	  REQUEST_SMARTD				=118;
   
   public static final int	  REQUEST_SMARTD_QUESTION				=119;
   
   public static final int	  REQUEST_SMARTD_SIMILARITY				=120;
   
   public static final int	  REQUEST_SMARTD_QUESTION_SELECTION				=121;
   
   public static final int	  REQUEST_SMARTD_NEIGHBOR_NUM_EVALUATION				=122;
   
   public static final int	  REQUEST_SMARTD_QUESTION_NUM_EVALUATION				=123;
   
   public static final int	  REQUEST_SMARTD_VALUE_DISTRIBUTION				=124;
   ...
   public void doGet ( HttpServletRequest request , HttpServletResponse response ) throws ServletException , IOException {
      ...
      case REQUEST_ONE_SHOT_QUERY :
         ...
         case REQUEST_ADDRESSING :
            ...
            break;
         case REQUEST_SMARTD :
        	 handler = new SmartDChartHandler ( );
             if ( handler.isValid ( request , response ) ) handler.handle ( request , response );
             break;
         case REQUEST_SMARTD_QUESTION :
        	 handler = new SmartDQuestionHandler ( );
             if ( handler.isValid ( request , response ) ) handler.handle ( request , response );
             break;
         case REQUEST_SMARTD_SIMILARITY :
        	 handler = new SmartDSimilarityHandler( );
             if ( handler.isValid ( request , response ) ) handler.handle ( request , response );
             break;
         case REQUEST_SMARTD_QUESTION_SELECTION :
        	 handler = new SmartDQuestionSelection( );
             if ( handler.isValid ( request , response ) ) handler.handle ( request , response );
             break;
         case REQUEST_SMARTD_NEIGHBOR_NUM_EVALUATION:
        	 handler = new SmartDNeighborNumEvaluation( );
             if ( handler.isValid ( request , response ) ) handler.handle ( request , response );
             break; 
         case REQUEST_SMARTD_QUESTION_NUM_EVALUATION :
        	 handler = new SmartDQuestionNumEvaluaton();
             if ( handler.isValid ( request , response ) ) handler.handle ( request , response );
             break;
         case REQUEST_SMARTD_VALUE_DISTRIBUTION :
        	 handler = new SmartDValueDistribution();
             if ( handler.isValid ( request , response ) ) handler.handle ( request , response );
             break;    
